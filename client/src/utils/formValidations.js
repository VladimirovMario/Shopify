export const formValidations = (e) => {
  ...
  if (
    target === 'description' &&
    (value.length < minLength + 1 || value.length > maxLength)
  ) {
    errors.description = `should be between ${
      minLength + 1
  ...
  if (
    target === 'subject' &&
    (value.length < minLength + 1 || value.length > middleLength)
  ) {
    errors.subject = `should be between ${
      minLength + 1
    } and ${middleLength} characters long`;
  }
  ...
  if (
    target === 'content' &&
    (value.length < minLength + 1 || value.length > 600)
  ) {
    errors.content = `should be between ${
      minLength + 1
    } and 600 characters long`;
  }
